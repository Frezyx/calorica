...
List<charts.Series<GraphLinarData, String>> createSampleData(List<UserProduct> weekStats) {

    List<GraphLinarData> tableSalesData = [];
    List<GraphLinarData> mobileSalesData = [];
    
    for (int i = 0; i < weekStats.length; i++){
      mobileSalesData.add(new GraphLinarData(i, weekStats[i].date.day.toString(), weekStats[i].calory));
      tableSalesData.add(new GraphLinarData(i, weekStats[i].date.day.toString(), weekStats[i].calory));
    }

    return [

      new charts.Series<GraphLinarData, String>(
        id: 'Tablet',
        colorFn: (_, __) => charts.ColorUtil.fromDartColor(DesignTheme.secondChartsGreen),
      ...
      new charts.Series<GraphLinarData, String>(
          id: 'Mobile',
          colorFn: (_, __) => charts.ColorUtil.fromDartColor(DesignTheme.secondColor),
          domainFn: (GraphLinarData data, _) => data.date,
