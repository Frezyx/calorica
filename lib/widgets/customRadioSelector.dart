...
class CustomRadioSelectorState extends State<CustomRadioSelector> {
  ...
  Widget build(BuildContext context) {
    return Stack(children: <Widget>[
      Padding(
        padding: EdgeInsets.only(top: 110, left: 30.0),
        child: Text(
          "Выберите свою цель: ",
          style: DesignTheme.selectorLabel,
        ),
      ),
      Padding(
          ...
          child: Column(
            children: <Widget>[
              ...
              GradientButton(
                ...
                callback: () {
                  DBUserProvider.db
                      .updateDateProducts("workFutureModel", workModel)
                      .then((count1) {
...
Future<void> _badAllert(context) async {
  return showDialog<void>(
    ...
    barrierDismissible: false,
    builder: (BuildContext context) {
      return AlertDialog(title: Text('Что-то пошло не так'), actions: <Widget>[
        FlatButton(
          child: Text('Ещё раз'),
          onPressed: () {
...
class RadioModel {
  ...
  RadioModel(this.isSelected, this.multiplaier, this.text, this.icon,
      this.title, this.subtitle, this.padding);
}
