...
class ErrorScreens {
  static Widget getErrorScreen(
    ...
    return Container(
      ...
      child: getScreenBody(imagePath, path, text, subText, buttonText,
          isButtonUsed, icon, context),
    );
  }

  static Widget getScreenBody(
      imagePath, path, text, subText, buttonText, isButtonUsed, icon, context) {
    return Column(
  ...
  static getNoMealScreen(context) {
    return getErrorScreen(
        "assets/svg/noMeal.svg",
        "/navigator/2",
        "Сегодня вы ничего не ели",
        "Добавьте прием пищи в меню поиска продукта",
        "Добавить",
        true,
        Icons.add,
        context);
